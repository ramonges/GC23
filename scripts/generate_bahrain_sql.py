...
def extract_company(operator):
    ...
    if 'saudi aramco' in op_lower or 'aramco' in op_lower:
        return "'Saudi Aramco'"
    if 'bapco' in op_lower:
...
json_path = os.path.join(script_dir, '../Bahrain_all_sites_with_coordinates.json')
...
for i, site in enumerate(deduplicated_sites):
  ...
  {escape_sql(f"{site['state_province']}, Bahrain")},
  'Bahrain',
  ...
  {company},
  '{{"source": "Bahrain_all_sites.json", "generated_date": "{data['generated_date']}"}}'::JSONB
)"""
    values.append(val)
...
# Write SQL file
output_path = os.path.join(script_dir, '../BAHRAIN_OIL_FIELDS_SETUP.sql')
with open(output_path, 'w', encoding='utf-8') as f:
    f.write(sql)

print(f'✅ Generated SQL file with {len(deduplicated_sites)} Bahrain sites (deduplicated from {len(data["sites"])})')
print(f'📁 Output: {output_path}')
print('\nNext steps:')
print('1. Run the SQL file in your Supabase SQL Editor')
print('2. Refresh your platform to see all Bahrain oil & gas fields on the globe')
